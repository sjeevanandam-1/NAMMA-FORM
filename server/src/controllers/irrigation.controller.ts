...
export class IrrigationController {
   ...
   */
  static async calculateIrrigation(req: AuthenticatedRequest, res: Response): Promise<void> {
    try {
      ...
      sendError(res, err.message || 'Failed to calculate smart irrigation', 500);
    }
  }
   ...
   */
  static async getMyIrrigationHistory(req: AuthenticatedRequest, res: Response): Promise<void> {
    try {
      const farmerId = req.user!.id;
      const history = await prisma.irrigationAdvisory.findMany({
        where: { farmerId },

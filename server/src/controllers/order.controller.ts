...
export class OrderController {
    ...
    try {
      ...
      const newOrder = await prisma.$transaction(async (tx) => {
        ...
        if (listing.availableQuantityKg < data.quantityKg) {
          throw new Error(
            `Requested quantity (${data.quantityKg} kg) exceeds available stock (${listing.availableQuantityKg} kg)`
          );
        }
      ...
      sendSuccess(res, { newOrder, paymentResult }, 'Order placed successfully and payment authorized', 201);
    } catch (err: any) {
      sendError(res, err.message || 'Order creation failed', 400);
    }
  }
   ...
   */
  static async getOrderDetail(req: AuthenticatedRequest, res: Response): Promise<void> {
    try {
      const { id } = req.params;
      const userId = req.user!.id;
  ...
  static async updateOrderStatus(req: AuthenticatedRequest, res: Response): Promise<void> {
    try {
      ...
      if (data.status === ORDER_STATUS.REJECTED && order.status !== ORDER_STATUS.REJECTED) {
      ...
      sendSuccess(res, updated, `Order status updated to ${data.status}`);
    } catch (err: any) {

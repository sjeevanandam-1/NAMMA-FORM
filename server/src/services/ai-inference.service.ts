...
export class AIModelNotInstalledError extends Error {
...
export class AIInferenceService {
  ...
  static async analyzeCrop(params: {
    ...
    let buffer: Buffer | null = params.fileBuffer || null;
    ...
    let result: CropAnalysisResult | null = null;
    let providerName = '';
    ...
    if (!result) {
      throw new AIModelNotInstalledError(
        'AI model not installed. Please start your custom AI model service (at AI_MODEL_SERVICE_URL in server/.env) or configure a valid AI model provider.'
      );
    }
   ...
   */
  private static async callCustomMLService(
    serviceUrl: string,
    buffer: Buffer,
    mimeType: string,
    selectedCrop?: string

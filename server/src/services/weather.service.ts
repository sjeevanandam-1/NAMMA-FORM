...
export class WeatherService {
   ...
   */
  static async getAgroWeather(district: string, state: string): Promise<AgroWeatherForecast> {
    ...
    try {
      const url = `https://api.weatherapi.com/v1/forecast.json?key=${ENV.WEATHER_API_KEY}&q=${encodeURIComponent(
        `${district}, ${state}`
      ...
      if (res.ok) {
        ...
        return {
          ...
          forecast: forecastList.map((f: any) => ({

...
class CalendarController {
    ...
    static async createCropCalendar(req, res) {
        try {
            ...
            if (cropName.toLowerCase().includes('tomato'))
                gestationDays = 90;
            else if (cropName.toLowerCase().includes('chilli'))
                gestationDays = 120;
            else if (cropName.toLowerCase().includes('onion'))
                gestationDays = 110;
            else if (cropName.toLowerCase().includes('paddy') || cropName.toLowerCase().includes('rice'))
                gestationDays = 140;
            else if (cropName.toLowerCase().includes('banana'))
                gestationDays = 330;
            const expectedHarvest = new Date(sowing.getTime() + gestationDays * 24 * 60 * 60 * 1000);
            const calendar = await prisma_js_1.prisma.cropCalendar.create({
                data: {
                    farmerId,
                    cropName,
            ...
            for (const t of taskTemplates) {
                await prisma_js_1.prisma.calendarTask.create({
                    data: {
            ...
            const fullCalendar = await prisma_js_1.prisma.cropCalendar.findUnique({
                where: { id: calendar.id },
                include: { tasks: { orderBy: { dueDate: 'asc' } } },
            });
            (0, response_js_1.sendSuccess)(res, fullCalendar, 'AI Crop Calendar created with stage tasks', 201);
        }
        catch (err) {
     ...
     */
    static async getMyCalendars(req, res) {
        try {
            const farmerId = req.user.id;
            const calendars = await prisma_js_1.prisma.cropCalendar.findMany({
                where: { farmerId },
                include: { tasks: { orderBy: { dueDate: 'asc' } } },
                orderBy: { createdAt: 'desc' },
            });
            (0, response_js_1.sendSuccess)(res, calendars);
        }
        catch (err) {
    ...
    static async toggleTaskStatus(req, res) {
        try {
            ...
            const task = await prisma_js_1.prisma.calendarTask.findUnique({
                where: { id: taskId },
            });
            if (!task) {
                (0, response_js_1.sendError)(res, 'Task not found', 404);
                return;
            }
            const updated = await prisma_js_1.prisma.calendarTask.update({
                where: { id: taskId },
                data: {
            ...
            (0, response_js_1.sendSuccess)(res, updated, 'Task status updated');
        }
        catch (err) {
            (0, response_js_1.sendError)(res, err.message, 400);
        }

...
class DiseaseService {
    ...
    static async analyzeImage(params) {
        ...
        if (!apiKey || apiKey.trim() === '') {
            throw new Error('AI Vision is not configured. Please configure GEMINI_API_KEY in server/.env.');
        }
...
Return ONLY a raw, valid JSON object matching this exact schema:
{
  ...
  "analysis": {
     ...
     */
    static async deleteScan(scanId, farmerId) {
     ...
     */
    static async getFarmerScans(farmerId) {
        return prisma_js_1.prisma.diseaseScan.findMany({
            where: { farmerId },

...
class AIService {
     ...
     */
    static async getPriceForecast(cropName, state = 'Tamil Nadu') {
        // Find latest market price
        const latest = await prisma_js_1.prisma.marketPrice.findFirst({
            where: {
        ...
        const advisoryNote = predicted14d > currentPrice
        ...
        // Save prediction
        if (crop) {
            await prisma_js_1.prisma.marketPrediction.create({
                data: {
                    cropId: crop.id,
                    currentPrice,
    ...
    static async calculateProfitAdvice(params) {
        const { farmerId, cropName, landAreaAcre, expectedYieldKg, productionCost, transportCost = 0, sellingPricePerKg, } = params;
        ...
        if (farmerId) {
            await prisma_js_1.prisma.profitPrediction.create({
                data: {
                    farmerId,
                    cropName,
                    landAreaAcre,
    ...
    static async getBestMarket(params) {
        ...
        const results = candidateMarkets.map((m) => {
            const transport = (0, distance_js_1.estimateTransportCost)(m.distanceKm, quantityKg);
        ...
        return {
            ...
            markets: results,
            verdict: `Although ${results[1]?.marketName || 'other markets'} offer higher raw price, ${bestMarket.marketName} delivers ₹${bestMarket.netReturn.toLocaleString()} highest NET profit after factoring ₹${bestMarket.transportCost} transport and mandi cess fees.`,
    ...
    static async handleAgriAIChat(params) {
        ...
        if (language === 'ta') {
            ...
            if (lower.includes('தக்காளி') || lower.includes('விலை') || lower.includes('விற்பனை')) {
                return {
                    response: 'வணக்கம்! இன்றைய சந்தை நிலவரப்படி, தக்காளியின் சராசரி விலை கிலோவிற்கு ₹28 முதல் ₹32 வரை உள்ளது. அடுத்த 10-14 நாட்களில் சந்தை வரத்து குறைவதால் விலை மேலும் 10-15% உயர வாய்ப்புள்ளது. உங்கள் அறுவடைக்கு முன் நம்ம பார்மில் (Namma Farm) முன்பதிவு செய்வது அதிக லாபம் தரும்.',
                    suggestedActions: [
            ...
            if (lower.includes('நோய்') || lower.includes('இலை') || lower.includes('மருந்து')) {
                return {
                    response: 'பயிரில் இலை சுருட்டை அல்லது புள்ளிகள் தெரிந்தால், எங்கள் AI பயிர் மருத்துவர் (AI Crop Doctor) மூலம் ஒரு புகைப்படத்தை பதிவேற்றவும். அது உடனடியாக சான்றளிக்கப்பட்ட இயற்கை மற்றும் பாதுகாப்பான தீர்வுகளை வழங்கும். பயிர் பாதுகாப்பு மருந்துகளைப் பயன்படுத்தும் முன் அதிகாரப்பூர்வ வழிகாட்டுதல்களைப் பின்பற்றவும்.',
                    suggestedActions: ['பயிர் புகைப்படத்தை ஸ்கேன் செய்யவும்', 'சான்றளிக்கப்பட்ட மருந்துகளைப் பார்க்கவும்'],
                    audioAvailable: true,
                };
            }
            return {
                response: 'வணக்கம் விவசாய நண்பரே! நான் நம்ம பார்ம் AI உதவியாளர் (Namma Farm AI Assistant). உங்கள் பயிர் தேர்வு, சந்தை விலை முன்னறிவிப்பு, நோய் மேலாண்மை மற்றும் நேரடி வாங்குபவர்களுடன் இணைக்க நான் உதவுகிறேன். நீங்கள் எதைப் பற்றி அறிய விரும்புகிறீர்கள்?',
        ...
        if (lower.includes('tomato') || lower.includes('when to sell') || lower.includes('price')) {
            return {
                response: 'Based on current APMC Mandi trends and AI price projections, tomato prices are currently ₹28–₹32/kg and expected to rise to ₹34–₹38/kg over the next 10 days. We recommend pre-listing your harvest on Namma Farm now to connect directly with bulk buyers at farmgate prices with zero middleman cuts.',
        ...
        if (lower.includes('disease') || lower.includes('leaf') || lower.includes('unhealthy') || lower.includes('pest')) {
            return {
                response: 'To diagnose crop health issues safely, open the "AI Crop Doctor" tab and capture or upload a clear photo of the infected leaf. Our vision engine will identify possible fungal or viral pathogens using ICAR/TNAU certified agricultural databases with verified organic and bio-treatment steps.',
        ...
        if (lower.includes('profit') || lower.includes('margin') || lower.includes('buyer')) {
            return {
                response: 'Direct farmgate selling through Namma Farm delivers an average 18% to 24% higher NET profit compared to traditional local auctions because buyers handle transportation and eliminate intermediary brokerage commissions.',
                suggestedActions: ['Calculate Net Margin in Profit Advisor', 'Compare Mandis vs Direct Buyers'],
                audioAvailable: true,
            };
        }
        return {
            response: "Hi! I'm Namma Farm AI Assistant. How can I help you with farming today?",
            suggestedActions: [
    ...
    static async chatWithAssistant(params) {
        ...
        const recentScans = await prisma_js_1.prisma.diseaseScan.findMany({
        ...
        let conversation = await prisma_js_1.prisma.aIConversation.findFirst({
            where: { userId },
            orderBy: { updatedAt: 'desc' },
        });
        if (!conversation) {
            conversation = await prisma_js_1.prisma.aIConversation.create({
                data: {
                    userId,
...
4. Keep answers concise, actionable, and formatted with bullet points for easy reading on mobile devices.`;
        ...
        await prisma_js_1.prisma.aIConversation.update({
    ...
    static async getConversationHistory(userId) {
        const conversation = await prisma_js_1.prisma.aIConversation.findFirst({
            where: { userId },
            orderBy: { updatedAt: 'desc' },
            include: {
                messages: {
    ...
    static async clearConversationHistory(userId) {
        const conversations = await prisma_js_1.prisma.aIConversation.findMany({
            where: { userId },
            select: { id: true },
        });
        for (const c of conversations) {
            await prisma_js_1.prisma.aIMessage.deleteMany({ where: { conversationId: c.id } });
            await prisma_js_1.prisma.aIConversation.delete({ where: { id: c.id } });
        }
